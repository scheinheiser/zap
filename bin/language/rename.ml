...
module Alpha : sig
  ...
  let rename_list ~(f : 'b VM.t -> 'a -> 'a * 'b VM.t) (env : 'b VM.t)
      (l : 'a list) : 'a list * 'b VM.t =
  ...
  let rec rename_pattern (env : (Ast.ident * bool) VM.t)
      ((loc, pat) : Ast.located_pattern) :
      Ast.located_pattern * (Ast.ident * bool) VM.t =
    let open Ast in
    match pat with
    | PConst (s, Ident i) ->
  ...
  let rec rename_expr (env : (Ast.ident * bool) VM.t)
      ((loc, expr) : Ast.located_expr) :
      Ast.located_expr * (Ast.ident * bool) VM.t =
    let open Ast in
    match expr with
    | Const (s, Ident i) -> (
      match VM.find_opt (Ast.get_str i) env with
      ...
      let bs =
        List.map
          (fun (p, wb, expr) ->
            ...
            let wb =
              Base.Option.map wb ~f:(Fun.compose fst (rename_expr env))
            in
            let expr, _ = rename_expr env expr in
            (p, wb, expr))
          bs
  ...
  let rec rename_definition (env : (Ast.ident * bool) VM.t)
      ((loc, d) : Ast.located_definition) :
      Ast.located_definition * (Ast.ident * bool) VM.t =
    let open Ast in
    match d with
    | Dec (hsd, i, sig') ->
      let i, env =
        if Ast.get_str i = "main" then
          (i, env)
        else
          let i = Ast.get_str i in
          let i' = fresh_alpha i in
