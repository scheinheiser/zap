...
*)
let rec unify (ctx: ctx) ((_, p) as p_with_loc: Ast.located_pattern) (((_, t') as t, ((loc, _) as e)): Typed_ast.typed_expr) : ctx Base.Or_error.t = 
  let uni_err loc pattern t =
    let msg = Format.asprintf "Failed to unify pattern '%s' with type %a." (Ast.show_pat pattern) Typed_ast.pp_expr t in
  ...
  | (Ast.PConst (_, Ast.Ident i)), _ -> Ok (extend ctx (Ast.get_str_combine i) ~t:t ~v:e)
  ...
  | (Ast.PConst (_, c)), Typed_ast.Const (_, c') ->
    ...
    if lc = rc
    then Ok ctx
    else uni_err loc p_with_loc t
  | (Ast.PConst (_, c)), Typed_ast.Bop (_, _, _) | (Ast.PConst (_, c)), Typed_ast.Ap (_, _, _) ->
    let _ = get_const_type c in failwith "finish normalise! for equality function"
  | (Ast.PCtor (i, ps)), Typed_ast.Ap (_, l, r) ->
    ...
    | "", _ | _, false -> make_err (Some loc, Format.asprintf "Expected a constructor, but got '%a'." Typed_ast.pp_typed_expr l)
    | i', _ ->
      let i = Ast.get_str_combine i in
      let cons =
      ...
      | _ when i <> i' -> make_err (Some loc, Printf.sprintf "Expected '%s' constructor, but got '%s'." i i')
      | _ ->
        let@ ctxs = List.map2 (fun p c -> unify ctx p c) ps cons |> Base.Or_error.combine_errors in
        List.rev ctxs |> List.hd))
  | _ -> uni_err loc p_with_loc t 

and normalise (ctx: ctx) ((t, e): Typed_ast.typed_expr): (Typed_ast.typed_expr * ctx) Base.Or_error.t =
  let open Typed_ast in
  let open Base.Or_error in
  let rec go ctx (loc, e) = 
    ...
    | Match (c, bs) ->
      ...
      (loc, Match (c, bs)), ctx
    | Lam (ps, b) -> (*NOTE: this may cause issues, keep an eye on it *)
      let@ b, _ = normalise ctx b in
      (loc, Lam (ps, b)), ctx
    | Pi (l, r) ->
...
let rec infer (ctx : ctx) ((loc, e) : Ast.located_expr) :
  ...
  | Binding (i, e) ->
    ...
    e', extend ctx (Ast.get_str_combine i) ~t:(fst e') ~v:(snd e') (* the identifier itself is associated with the bound type though *)
  | Pi (l, r) ->
    ...
    let@ u1, u2 =
      ...
      | _ -> make_err (Some loc, Format.asprintf "Expected type, got '%a'." Ast.pp_expr (loc, e))
    in
