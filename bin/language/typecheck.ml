...
let rec get_pattern_type (env : env) ((loc, pat) : Ast.located_pattern)
  ...
  | PCons (l, r) ->
    ...
    (match r' with
     ...
     | List (_, t) ->
       unify env (loc1, l') (loc2, r') >>| fun env -> (loc', List (loc', t)), env
     | t ->
       make_err
         ( Some loc'
...
let rec check_expr (env : env) (e : Ast.located_expr)
  ...
  =
  let open Base.Or_error in
  check_expr' env e >>| fun (e, env) -> sub_expr env e, env
...
let rec check_term (env : env) (t : Ast.located_term)
  : (Typed_ast.typed_term * env) Base.Or_error.t
  =
  let open Base.Or_error in
  check_term' env t >>| fun (t, env) -> sub_term env t, env
  ...
  | TIf (cond, texpr, fexpr) ->
    ...
    (match fexpr with
     ...
     | Some fexpr ->
       ...
       let env = replace_tyvars env e.tyvar_env in
       unify env tty fty
       >>= fun env -> Ok (((loc, t), (loc, Typed_ast.TIf (cond, texpr, Some fexpr))), env))
  | TLam (args, body) ->
    ...
    | Some dec_ty ->
      let rec unify_arg
        ...
        | PConst c ->
          ...
          unify env (ty_loc, l) c >>= fun env -> Ok ((ty_loc, l), env)
