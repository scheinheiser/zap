...
and expr =
  ...
  | Match of
    typed_expr * (Ast.located_pattern * typed_expr option * typed_expr) list
  | TypeLit of Ast.prim
...
let rec pp_expr out ((_, e) : located_expr) =
  ...
  | List l ->
    Format.fprintf out "([@[<hov>%a@]])"
      Format.(
        pp_print_list ~pp_sep:(fun out () -> fprintf out ";@ ") pp_typed_expr)
      l
  | Ap (_, f, arg) ->
    Format.fprintf out "(@[<hov>%a@ %a@])" pp_typed_expr f pp_typed_expr arg
  | Bop (l, op, r) ->
    Format.fprintf out "(@[<hov>%a@ %a@ %a@])" Ast.pp_binop op pp_typed_expr l
      pp_typed_expr r
  | Let (p, v, n) ->
    Format.fprintf out "(le@[<v>t %a %a@,%a@])" Ast.pp_pattern p pp_typed_expr v
      pp_typed_expr n
  | If (cond, tbranch, fbranch) ->
    Format.fprintf out "(if@[<v> %a@,%a@,%a@])" pp_typed_expr cond pp_typed_expr
      tbranch
      Format.(
        pp_print_option ~none:(fun out () -> fprintf out "<none>") pp_typed_expr)
      fbranch
  | Lam (args, body) ->
    Format.fprintf out "(la@[<v>m (%a)@,%a@])"
      Format.(
        pp_print_list ~pp_sep:(fun out () -> fprintf out " ") Ast.pp_pattern)
      args pp_typed_expr body
  | Match (cond, bs) ->
    let pp_branch out (p, wb, b) =
      Format.fprintf out "(wh@[<v>en %a@,%a %a@])"
        Format.(
          pp_print_option ~none:(fun out () -> fprintf out "true") pp_typed_expr)
        wb Ast.pp_pattern p pp_typed_expr b
    in
    Format.fprintf out "(ma@[<v>tch (%a)@,%a@])" pp_typed_expr cond
...
let pp_typed_definition out
    ((_, (_, f, ret, args, when_block, body)) : located_definition) =
  Format.fprintf out "(de@[<v>f %a (%a)@,(%a)@,%a@,%a@])" Ast.pp_ident f
    pp_typed_expr ret
    Format.(
      pp_print_list ~pp_sep:(fun out () -> fprintf out " ") Ast.pp_pattern)
    args pp_when_block when_block pp_typed_expr body

let pp_typed_program out ((prog_name, imports, types, body) : program) =
  Format.fprintf out "%a@.@.%a@.@.%a@.@.%a@." Ast.pp_module prog_name
    Format.(
      pp_print_list ~pp_sep:(fun out () -> fprintf out "@.") Ast.pp_import)
    imports
    Format.(
      pp_print_list ~pp_sep:(fun out () -> fprintf out "@.") Ast.pp_ty_decl)
    types
    Format.(
      pp_print_list ~pp_sep:(fun out () -> fprintf out "@.") pp_typed_definition)
    body
